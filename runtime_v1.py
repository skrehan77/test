...
BotFlow Runtime v2
==================
A simple DSL engine for building Telegram bots fast.
...
flow weather
  input city Which city?
  fetch data https://wttr.in/{city}?format=3
  send Weather: {data}
  go menu

flow joke
  fetch joke https://official-joke-api.appspot.com/random_joke
  send {joke.setup}
  send {joke.punchline}
  go menu
...
Supported commands:
  send <text>               → send a message (supports {var} substitution)
  input <var> <prompt>      → ask the user for text input, store in <var>
  select <var>              → show inline buttons; next lines: Label = value
    Label = value
  fetch <var> <url>         → GET a URL (JSON), store result in <var>
  if <expr>                 → skip next block if false; supports ==, !=, contains
    <command>               → one or more commands (can be nested)
  go <flow>                 → jump to another flow
  set <var> = <value>       → set a variable (supports {var} substitution in value)
  delay <seconds>           → wait N seconds before continuing
  image <url>               → send an image
  buttons <prompt>          → send inline URL buttons; next lines: Label = url
    Label = url

Dot notation for nested JSON:
  fetch res https://api.example.com/data
  send {res.user.name}   → accesses res["user"]["name"]
"""
